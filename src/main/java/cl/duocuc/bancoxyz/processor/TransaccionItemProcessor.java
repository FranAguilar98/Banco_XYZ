...
public class TransaccionItemProcessor implements ItemProcessor<TransaccionCsv, Transaccion> {
    ...
    public Transaccion process(TransaccionCsv item) {
        ...
        if (partitionIndex != null && gridSize != null) {
            int claveReparto = calcularClaveReparto(item);
    ...
    private int calcularClaveReparto(TransaccionCsv item) {
        Long id = parseIdSeguro(item.getId());
        return id != null ? id.intValue() : item.toString().hashCode();
    }
